...
def subscribe(user: ArtifactUser, subscription_pk: int) -> (bool, str):
    try:
    ...
    current_subscription: SponsorshipSubscription = get_current_user_subscription(user, subscription_type.owner)
...
def unsubscribe(user: ArtifactUser, subscription_pk: int) -> (bool, str):
    try:
...
def get_current_user_subscription(subscriber: ArtifactUser, subscription_owner: ArtifactUser) -> Optional[SponsorshipSubscription]:
    try:
